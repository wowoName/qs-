<template>
  <div class="indexMain">
    ...
    <div class="userInfos">
      ...
      <div class="userNames">
        <div class="userName">
          ...
          <div>{{usrInfos.user.level_cn||"萌新"}}</div>
        </div>
    ...
    <div class="balance">
      ...
      <div>余额（元）</div>
      <div @click="$router.push({name:'withDrawal',params:{amount:usrInfos.user.user_money}})">提现</div>
    </div>
    ...
    getUserInfo() {
      this.ajax.get(
        ...
        data => {
          ...
          //保存当前用户的等级
          window.localStorage.setItem("liveLevel", data.data.datas.user.level);
          this.$store.commit("setLevel", data.data.datas.user.level);
        },
        data => { }
      );
    }
...
.indexMain {
  ...
  .userInfos {
    ...
    background: url(../../static/images/indexBg.png) no-repeat center
      center/100% 100%;
  ...
  .incomeDetails {
    ...
    .entrance {
      width: 100%;
      @include indexFlex(center, space-around);
      font-size: 28px;
      color: rgba(0, 0, 0, 1);
