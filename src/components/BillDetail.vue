<template>
	<div class="bDeMain">
		...
		<div class="billDeCom">
			<view-box>
				<div class="billDeItem" v-for="(item,index) in bdData" :key="item.balance_id">
					<span>{{item.time}}</span>
					<span>{{item.type_sn}}</span>
					<span v-if="item.type=='cashback' " style="color:#FE1969">+{{item.cost}}</span>
					<span v-else style="color:#009E03">-{{item.cost}}</span>
				</div>
			</view-box>
		</div>
